...
assert.match(audio, /hoverMenuPendiente = true/, 'el primer hover no queda pendiente');
assert.match(audio, /contexto\.resume\(\)/, 'no se reanuda directamente el contexto de audio');
assert.match(audio, /if \(sobreJugar\) comenzarDesdeBienvenida\(\)/, 'el botón de bienvenida no habilita el audio');
assert.match(audio, /if \(!contextoAudioListo\(\)\) return false/, 'el audio se marca listo antes de que el contexto esté activo');
assert.match(audio, /hoverMenuPendiente && estadoJuego === ESTADOS\.MENU/, 'el hover pendiente puede sonar fuera del menú');
